...
DROP TABLE IF EXISTS  	orig_ego.ego_deu_loads_spf CASCADE;
CREATE TABLE         	orig_ego.ego_deu_loads_spf (
		id SERIAL NOT NULL,
		geom geometry(Polygon,3035),
CONSTRAINT 	ego_deu_loads_spf_pkey PRIMARY KEY (id));

-- "Insert Loads"   (ERROR!) 10.000ms =967
INSERT INTO     orig_ego.ego_deu_loads_spf (geom)
	SELECT	(ST_DUMP(ST_INTERSECTION(loads.geom,dis.geom))).geom ::geometry(Polygon,3035) AS geom
	FROM	orig_ego.ego_deu_loads_melted_spf AS loads,
...
DROP MATERIALIZED VIEW IF EXISTS	orig_ego.ego_deu_loads_spf_error_area_ha_mview CASCADE;
CREATE MATERIALIZED VIEW 		orig_ego.ego_deu_loads_spf_error_area_ha_mview AS 
...
-- "Grant oeuser"   (OK!) -> 100ms =0
GRANT ALL ON TABLE 	orig_ego.ego_deu_loads_spf_error_area_ha_mview TO oeuser WITH GRANT OPTION;
...
DROP MATERIALIZED VIEW IF EXISTS	orig_ego.ego_deu_loads_spf_centre_mview CASCADE;
CREATE MATERIALIZED VIEW 		orig_ego.ego_deu_loads_spf_centre_mview AS 
	SELECT	loads.id AS id,
...
-- "Grant oeuser"   (OK!) -> 100ms =0
GRANT ALL ON TABLE 	orig_ego.ego_deu_loads_spf_centre_mview TO oeuser WITH GRANT OPTION;
ALTER TABLE		orig_ego.ego_deu_loads_spf_centre_mview OWNER TO oeuser;
...
FROM    (
	...
	FROM	orig_osm.osm_deu_polygon_urban_sector_1_residential_mview AS sector,
		orig_ego.ego_deu_loads_spf AS loads	
