...
DROP TABLE IF EXISTS 	model_draft.ego_supply_res_powerplant_per_gentype_and_voltlevel CASCADE;
CREATE TABLE 		model_draft.ego_supply_res_powerplant_per_gentype_and_voltlevel AS
...
ALTER TABLE	model_draft.ego_supply_res_powerplant_per_gentype_and_voltlevel
	ADD PRIMARY KEY (id),
	OWNER TO oeuser;

-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.3','output','model_draft','ego_supply_res_powerplant_per_gentype_and_voltlevel','ego_rea_results.sql',' ');
...
*/ 
/*  -- integrate in MVGD
DROP TABLE IF EXISTS 	model_draft.ego_supply_res_powerplant_per_mvgd CASCADE;
CREATE TABLE 		model_draft.ego_supply_res_powerplant_per_mvgd AS
	SELECT	gd.subst_id,
		'0'::integer dea_cnt,
...
ALTER TABLE	model_draft.ego_supply_res_powerplant_per_mvgd
	ADD PRIMARY KEY (subst_id),
	OWNER TO oeuser;
		
-- create index GIST (geom)
CREATE INDEX ego_supply_res_powerplant_per_mvgd_geom_idx
	ON model_draft.ego_supply_res_powerplant_per_mvgd USING gist (geom);

-- grant (oeuser)
ALTER TABLE model_draft.ego_supply_res_powerplant_per_mvgd OWNER TO oeuser;  
 */
...
*/ 
DROP TABLE IF EXISTS 	model_draft.ego_supply_res_powerplant_per_loadarea CASCADE;
CREATE TABLE 		model_draft.ego_supply_res_powerplant_per_loadarea AS
	SELECT	la.id,
		la.subst_id,
		'0'::integer lv_dea_cnt,
		'0.0'::decimal lv_dea_capacity
...
ALTER TABLE	model_draft.ego_supply_res_powerplant_per_loadarea
	ADD PRIMARY KEY (id),
	OWNER TO oeuser;

UPDATE 	model_draft.ego_supply_res_powerplant_per_loadarea AS t1
SET  	lv_dea_cnt = t2.lv_dea_cnt,
	lv_dea_capacity = t2.lv_dea_capacity
...
FROM	model_draft.ego_supply_res_powerplant_per_loadarea AS la,
	model_draft.ego_grid_mv_griddistrict AS gd;
*/

-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.3','output','model_draft','ego_supply_res_powerplant_per_loadarea','ego_rea_results.sql',' ');


/* 
DEA capacity and count per load area
*/ 
DROP TABLE IF EXISTS 	model_draft.ego_supply_res_powerplant_per_method CASCADE;
CREATE TABLE 		model_draft.ego_supply_res_powerplant_per_method AS
	SELECT	'all' AS name,
...
ALTER TABLE	model_draft.ego_supply_res_powerplant_per_method
	ADD PRIMARY KEY (name),
	OWNER TO oeuser;

-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.3','output','model_draft','ego_supply_res_powerplant_per_method','ego_rea_results.sql',' ');
