...
SELECT ego_scenario_log('v0.2.9','input','supply','vernetzen_wind_potential_area','ego_dp_rea_wpa_per_mvgd.sql',' ');
...
-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.9','temp','model_draft','ego_supply_wpa','ego_dp_rea_wpa_per_mvgd.sql',' ');
 */
/* -- validate (geom)
...
-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.9','input','model_draft','ego_grid_mv_griddistrict','ego_dp_rea_wpa_per_mvgd.sql',' ');
SELECT ego_scenario_log('v0.2.9','input','supply','vernetzen_wind_potential_area','ego_dp_rea_wpa_per_mvgd.sql',' ');
...
COMMENT ON TABLE model_draft.ego_supply_wpa_per_mvgd IS '{
	...
	"reference_date": "",
	"sources": [
		{"name": "eGo dataprocessing", "description": "v0.2.9", "url": "https://github.com/openego/data_processing", "license": "GNU Affero General Public License Version 3 (AGPL-3.0)", "copyright": " " },
	...
	"contributors": [
		...
		{"name": "Ludwig Hülk", "email": "ludwig.huelk@rl-institut.de",
		"date": "28.03.2017", "comment": "Update to v0.2.9"}],
	"resources": [{
		"schema": {
			"fields": [
...
-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.9','output','model_draft','ego_supply_wpa_per_mvgd','ego_dp_rea_wpa_per_mvgd.sql',' ');
