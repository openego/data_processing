/*
M33 wind
Allocates wind power stations to corresponding OSM locations. Generously ignores complexity of old M3-Method. Hence the new name M33.
	...
	LOOP
		...
		INSERT INTO sandbox.ego_reaosm_m3_osm_temp
			SELECT 	COALESCE((
                        ...
                        FROM    sandbox.ego_reaosm_m3_osm_temp), ''0'') 
                            + row_number() over (ORDER BY area_type DESC) AS reaosm_sort,
				    wpa.id,
				    wpa.subst_id,
				    wpa.area_type,
				    wpa.geom ::geometry(Point,3035) AS geom
