...
SELECT ego_scenario_log('v0.2.6','input','model_draft','ego_supply_conv_powerplant_2035','ego_dp_conv_2035.sql',' ');
...
UPDATE 	model_draft.ego_supply_conv_powerplant_2035 AS t1
	...
	FROM    (
		SELECT	a.gid AS id,
			b.id AS la_id
		FROM	model_draft.ego_supply_conv_powerplant_2035 AS a,
...
-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.6','output','model_draft','ego_supply_conv_powerplant_2035','ego_dp_conv_2035.sql',' ');
