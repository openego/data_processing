...
SELECT ego_scenario_log('v0.2.9','input','model_draft','ego_supply_res_powerplant_2050','ego_dp_rea_setup_2050.sql',' ');
...
-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.9','input','model_draft','ego_grid_mv_griddistrict','ego_dp_rea_setup_2050.sql',' ');
...
UPDATE 	model_draft.ego_supply_res_powerplant_2050 AS t1
	...
	WHERE  	t1.id = t2.id;
-- rea_flag reset
UPDATE 	model_draft.ego_supply_res_powerplant_2050
	SET	rea_flag = NULL,
		rea_geom_new = NULL,
		rea_geom_line = NULL;
...
-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.9','output','model_draft','ego_supply_res_powerplant_2050','ego_dp_rea_setup_2050.sql',' ');
...
-- re outside mv-griddistrict
DROP MATERIALIZED VIEW IF EXISTS 	model_draft.ego_supply_res_powerplant_2050_out_mview CASCADE;
CREATE MATERIALIZED VIEW 		model_draft.ego_supply_res_powerplant_2050_out_mview AS
	SELECT	*
	FROM 	model_draft.ego_supply_res_powerplant_2050
	WHERE	rea_flag = 'out' OR rea_flag = 'wind_offshore';

-- index GIST (geom)
CREATE INDEX ego_supply_res_powerplant_2050_out_mview_geom_idx
	ON model_draft.ego_supply_res_powerplant_2050_out_mview USING gist (geom);

-- index GIST (rea_geom_line)
CREATE INDEX ego_supply_res_powerplant_2050_out_mview_rea_geom_line_idx
	ON model_draft.ego_supply_res_powerplant_2050_out_mview USING gist (rea_geom_line);

-- index GIST (rea_geom_new)
CREATE INDEX ego_supply_res_powerplant_2050_out_mview_rea_geom_new_idx
	ON model_draft.ego_supply_res_powerplant_2050_out_mview USING gist (rea_geom_new);	

-- grant (oeuser)
ALTER TABLE model_draft.ego_supply_res_powerplant_2050_out_mview OWNER TO oeuser;

-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.9','temp','model_draft','ego_supply_res_powerplant_2050_out_mview','ego_dp_rea_setup_2050.sql','First check if RES are outside Germany');


-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.9','input','model_draft','ego_grid_hvmv_substation','ego_dp_rea_setup_2050.sql',' ');

-- new geom, DEA to next substation
DROP TABLE IF EXISTS	model_draft.ego_supply_res_powerplant_2050_out_nn CASCADE;
CREATE TABLE 		model_draft.ego_supply_res_powerplant_2050_out_nn AS 
	...
	FROM 	model_draft.ego_supply_res_powerplant_2050_out_mview AS a,
...
ALTER TABLE	model_draft.ego_supply_res_powerplant_2050_out_nn
	ADD PRIMARY KEY (dea_id),
	OWNER TO oeuser;

-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.9','temp','model_draft','ego_supply_res_powerplant_2050_out_nn','ego_dp_rea_setup_2050.sql',' ');
...
UPDATE 	model_draft.ego_supply_res_powerplant_2050 AS t1
	...
	FROM	(SELECT	nn.dea_id AS dea_id,
		...
		FROM	model_draft.ego_supply_res_powerplant_2050_out_nn AS nn,
			model_draft.ego_supply_res_powerplant_2050 AS dea
		WHERE  	rea_flag = 'out'
		)AS t2
	WHERE  	t1.id = t2.dea_id;
	
-- time log: 01:34:3650 hours
...
-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.9','temp','model_draft','ego_supply_res_powerplant_2050_out_mview','ego_dp_rea_setup_2050.sql','Second check if RES outside Germany');
...
/*
-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.9','input','model_draft','ego_osm_sector_per_griddistrict_4_agricultural','ego_dp_rea_setup_2050.sql',' ');
