...
DROP TABLE IF EXISTS  	calc_ego_loads.ego_deu_load_area CASCADE;
CREATE TABLE         	calc_ego_loads.ego_deu_load_area (
		id SERIAL NOT NULL,
...
CREATE INDEX  	ego_deu_load_area_geom_idx
    ON    	calc_ego_loads.ego_deu_load_area
    USING     	GIST (geom);

-- "Grant oeuser"   (OK!) -> 100ms =0
GRANT ALL ON TABLE 	calc_ego_loads.ego_deu_load_area TO oeuser WITH GRANT OPTION;
ALTER TABLE		calc_ego_loads.ego_deu_load_area OWNER TO oeuser;
...
DROP MATERIALIZED VIEW IF EXISTS	calc_ego_loads.ego_deu_load_area_error_area_ha_mview CASCADE;
CREATE MATERIALIZED VIEW 		calc_ego_loads.ego_deu_load_area_error_area_ha_mview AS 
...
-- "Grant oeuser"   (OK!) -> 100ms =0
GRANT ALL ON TABLE 	calc_ego_loads.ego_deu_load_area_error_area_ha_mview TO oeuser WITH GRANT OPTION;
ALTER TABLE		calc_ego_loads.ego_deu_load_area_error_area_ha_mview OWNER TO oeuser;

-- "Remove Errors (area_ha)"   (OK!) 700ms =1.850
DELETE FROM	calc_ego_loads.ego_deu_load_area AS loads
	WHERE	loads.area_ha < 0.001;
...
DROP MATERIALIZED VIEW IF EXISTS	calc_ego_loads.ego_deu_load_area_centre_mview CASCADE;
CREATE MATERIALIZED VIEW 		calc_ego_loads.ego_deu_load_area_centre_mview AS 
	SELECT	loads.id AS id,
...
-- "Grant oeuser"   (OK!) -> 100ms =0
GRANT ALL ON TABLE 	calc_ego_loads.ego_deu_load_area_centre_mview TO oeuser WITH GRANT OPTION;
ALTER TABLE		calc_ego_loads.ego_deu_load_area_centre_mview OWNER TO oeuser;
...
FROM    (
	...
	FROM	calc_ego_loads.ego_deu_load_area AS load
	) AS t2
WHERE  	t1.id = t2.id;
...
DROP MATERIALIZED VIEW IF EXISTS	calc_ego_loads.ego_deu_load_area_error_noags_mview CASCADE;
CREATE MATERIALIZED VIEW 		calc_ego_loads.ego_deu_load_area_error_noags_mview AS 
	SELECT	loads.id,
		loads.geom
	FROM	calc_ego_loads.ego_deu_load_area AS loads
...
UPDATE 	calc_ego_loads.ego_deu_load_area AS a
...
-- Ad PK   (OK!) 150ms =0
ALTER TABLE	calc_ego_loads.ego_deu_load_area_ta
	ADD PRIMARY KEY (id);

-- Create Index GIST (geom)   (OK!) 2.500ms =0
CREATE INDEX	ego_deu_load_area_ta_geom_idx
	ON	calc_ego_loads.ego_deu_load_area_ta
	USING	GIST (geom);

-- Grant oeuser   (OK!) -> 100ms =0
GRANT ALL ON TABLE	calc_ego_loads.ego_deu_load_area_ta TO oeuser WITH GRANT OPTION;
ALTER TABLE		calc_ego_loads.ego_deu_load_area_ta OWNER TO oeuser;
...
-- DROP TABLE IF EXISTS  	calc_ego_loads.ego_deu_load_area_spf;
...
-- 
-- -- "Ad PK"   (OK!) 150ms =0
-- ALTER TABLE	calc_ego_loads.ego_deu_load_area_spf
-- 	ADD PRIMARY KEY (id);
-- 
-- -- "Create Index GIST (geom)"   (OK!) -> 100ms =0
-- CREATE INDEX  	ego_deu_load_area_spf_geom_idx
-- 	ON	calc_ego_loads.ego_deu_load_area_spf
-- 	USING	GIST (geom);
-- 
...
-- 
-- -- "Grant oeuser"   (OK!) -> 100ms =0
-- GRANT ALL ON TABLE 	calc_ego_loads.ego_deu_load_area_spf TO oeuser WITH GRANT OPTION;
-- ALTER TABLE		calc_ego_loads.ego_deu_load_area_spf OWNER TO oeuser;