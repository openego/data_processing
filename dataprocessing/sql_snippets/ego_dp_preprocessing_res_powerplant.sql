...
SELECT ego_scenario_log('v0.2.8','input','supply','ego_renewable_powerplant','ego_dp_preprocessing_res_powerplant.sql','');
...
-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.8','temp','model_draft','ego_supply_res_powerplant','ego_dp_preprocessing_res_powerplant.sql','');
...
UPDATE model_draft.ego_supply_res_powerplant
 	SET 	voltage_level=
		(CASE 
		 	WHEN voltage_level_var='01 (HöS)' 
		 	THEN 1 
		 	WHEN voltage_level_var='02 (HöS/HS)'
		 	THEN 2 
		 	WHEN voltage_level_var='03 (HS)'
		 	THEN 3
		 	WHEN voltage_level_var='04 (HS/MS)'
		 	THEN 4
		 	WHEN voltage_level_var='05 (MS)'
		 	THEN 5
		 	WHEN voltage_level_var='06 (MS/NS)'
		 	THEN 6 
		 	WHEN voltage_level_var='07 (NS)'
		 	ELSE NULL
		 END);
...
-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.8','output','model_draft','ego_supply_res_powerplant','ego_dp_preprocessing_res_powerplant.sql','');
