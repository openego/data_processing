...
DROP VIEW IF EXISTS calc_ego_substation.substation_hoes CASCADE;
...
SELECT openstreetmap.osm_deu_rels.id, openstreetmap.osm_deu_rels.tags, st_relation_geometry(openstreetmap.osm_deu_rels.members) as way
...
CREATE VIEW calc_ego_substation.substation_hoes AS
SELECT  *,
...
SELECT  ST_X(ST_Centroid(ST_Transform(substation.way,4326))) as lon,
	...
	(CASE WHEN hstore(substation.tags)->'operator' in ('DB_Energie','DB Netz AG','DB Energie GmbH','DB Netz')
	      ...
	      END) as dbahn,
	status
FROM calc_ego_substation.substation_hoes substation ORDER BY osm_www;
