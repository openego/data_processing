...
UPDATE model_draft.ego_dp_supply_conv_powerplant a
    ...
    WHERE   a.geom && ST_TRANSFORM(b.geom,4326)
        AND ST_Contains(a.geom, ST_TRANSFORM(b.geom,4326)) 
        AND voltage_level >= 3; 
...
SELECT scenario_log('eGo_DP', 'v0.4.1','input','model_draft','ego_dp_supply_conv_powerplant','ego_dp_substation_id_to_generator.sql',' ');
