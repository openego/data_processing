...
-- "Create Table"   (OK!) 100ms =0
DROP TABLE IF EXISTS	orig_geo_ego.ego_deu_loads_buffer100_unbuffer;
CREATE TABLE	orig_geo_ego.ego_deu_loads_buffer100_unbuffer (
		id SERIAL,
		geom geometry(Polygon,3035),
CONSTRAINT	ego_deu_loads_buffer100_unbuffer_pkey PRIMARY KEY (id));

-- "Insert Buffer"   (OK!) 350.000ms =202.130
INSERT INTO	orig_geo_ego.ego_deu_loads_buffer100_unbuffer (geom)
	SELECT	(ST_DUMP(ST_MULTI(ST_UNION(
			ST_BUFFER(ST_TRANSFORM(buffer.geom,3035), -100)
		)))).geom ::geometry(Polygon,3035) AS geom
	FROM	orig_geo_ego.ego_deu_loads_buffer100 AS buffer
	GROUP BY buffer.id
	ORDER BY buffer.id;

-- "Create Index GIST (geom)"   (OK!) 2.000ms =0
CREATE INDEX	ego_deu_loads_buffer100_unbuffer_geom_idx
	ON	orig_geo_ego.ego_deu_loads_buffer100_unbuffer
	USING	GIST (geom);

-- "Grant oeuser"   (OK!) -> 100ms =0
GRANT ALL ON TABLE	orig_geo_ego.ego_deu_loads_buffer100_unbuffer TO oeuser WITH GRANT OPTION;
ALTER TABLE		orig_geo_ego.ego_deu_loads_buffer100_unbuffer OWNER TO oeuser;
...
DROP TABLE IF EXISTS	orig_geo_ego.ego_deu_loads_buffer100_unbuffer_cut;
CREATE TABLE	orig_geo_ego.ego_deu_loads_buffer100_unbuffer_cut (
		id SERIAL,
		geom geometry(Polygon,3035),
CONSTRAINT	ego_deu_loads_buffer100_unbuffer_cut_pkey PRIMARY KEY (id));

-- "Insert Cut"   (OK!) 350.000ms =
INSERT INTO	orig_geo_ego.ego_deu_loads_buffer100_unbuffer_cut (geom)
	SELECT	ST_INTERSECTION(poly.geom,cut.geom) ::geometry(Polygon,3035) AS geom
	FROM	orig_geo_ego.ego_deu_loads_buffer100_unbuffer AS poly,
		calc_gridcells_znes.znes_deu_gridcells_qgis AS cut
	GROUP BY poly.id
	ORDER BY poly.id;

-- "Create Index GIST (geom)"   (OK!) 2.000ms =0
CREATE INDEX	ego_deu_loads_buffer100_unbuffer_cut_geom_idx
	ON	orig_geo_ego.ego_deu_loads_buffer100_unbuffer_cut
	USING	GIST (geom);
