...
DROP TABLE IF EXISTS	orig_ego.ego_deu_loads_zensus;
CREATE TABLE 		orig_ego.ego_deu_loads_zensus AS
...
DELETE FROM	orig_ego.ego_deu_loads_zensus AS lp
	WHERE	lp.inside_la IS TRUE;

-- "Grant oeuser"   (OK!) -> 1.000ms =693.968
GRANT ALL ON TABLE 	orig_ego.ego_deu_loads_zensus TO oeuser WITH GRANT OPTION;
ALTER TABLE		orig_ego.ego_deu_loads_zensus OWNER TO oeuser;
...
DROP TABLE IF EXISTS	orig_ego.ego_deu_loads_zensus_cluster CASCADE;
CREATE TABLE         	orig_ego.ego_deu_loads_zensus_cluster (
	cid serial,
	zensus_sum INT,
	area_ha INT,
...
FROM    (
	...
	FROM	orig_ego.ego_deu_loads_zensus AS lp,
		orig_ego.ego_deu_loads_zensus_cluster AS cl
...
GRANT ALL ON TABLE 	orig_ego.ego_deu_loads_zensus_cluster TO oeuser WITH GRANT OPTION;
ALTER TABLE		orig_ego.ego_deu_loads_zensus_cluster OWNER TO oeuser;
...
-- DROP TABLE IF EXISTS  	orig_ego.ego_deu_loads_zensus_cluster_spf;
-- CREATE TABLE         	orig_ego.ego_deu_loads_zensus_cluster_spf AS
-- 	SELECT	lp.*
-- 	FROM	orig_ego.ego_deu_loads_zensus_cluster AS lp,
...
-- 
-- -- "Ad PK"   (OK!) 150ms =0
-- ALTER TABLE	orig_ego.ego_deu_loads_zensus_cluster_spf
-- 	ADD PRIMARY KEY (cid);
-- 
-- -- "Create Index GIST (geom)"   (OK!) -> 100ms =0
-- CREATE INDEX  	ego_deu_loads_zensus_cluster_spf_geom_idx
-- 	ON	orig_ego.ego_deu_loads_zensus_cluster_spf
-- 	USING	GIST (geom);
-- 
...
-- 
-- -- "Grant oeuser"   (OK!) -> 100ms =0
-- GRANT ALL ON TABLE 	orig_ego.ego_deu_loads_zensus_cluster_spf TO oeuser WITH GRANT OPTION;
-- ALTER TABLE		orig_ego.ego_deu_loads_zensus_cluster_spf OWNER TO oeuser;
