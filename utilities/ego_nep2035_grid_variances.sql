...
SELECT 'extension_NEP', nextval('model_draft.ego_grid_hv_extension_bus_id'), geom, current_type, v_nom, bus_name, project FROM model_draft.ego_grid_pf_hv_extension_bus WHERE bus_name = 'Wilster';
...
SELECT nextval('model_draft.ego_grid_hv_extension_transformer_id'),
	...
	bus1,
	v_nom
FROM model_draft.ego_grid_pf_hv_extension_link a
WHERE bus1 NOT IN (SELECT bus_id FROM model_draft.ego_grid_pf_hv_extension_bus WHERE bus_name IN ('center BE', 'center NO'))
AND bus1 NOT IN (SELECT bus0 FROM model_draft.ego_grid_pf_hv_extension_link WHERE a.scn_name = scn_name) 
AND bus1 NOT IN (SELECT bus_id FROM model_draft.ego_grid_pf_hv_bus);
...
UPDATE model_draft.ego_grid_pf_hv_extension_transformer a
SET 	v0 = (CASE WHEN v0= 0 THEN (SELECT v_nom FROM model_draft.ego_grid_pf_hv_bus b
WHERE b.v_nom != 110 AND a.bus1 = b.bus_id AND b.scn_name = 'NEP 2035') ELSE v0 END),
...
SELECT  scn_name, nextval('model_draft.ego_grid_hv_extension_line_id'), bus0, bus1, geom, topo, v0, ST_Length(ST_GeographyFromText(ST_AsEWKT(geom)))/1000, s_min
...
INSERT INTO model_draft.ego_grid_pf_hv_extension_transformer (trafo_id, bus1, v1, capital_cost)
(SELECT DISTINCT ON (bus_id)
	nextval('model_draft.ego_grid_hv_extension_transformer_id'),  
	bus_id,
	b.v_nom,
	(CASE b.v_nom WHEN 110 THEN 1010.15 WHEN 220 THEN 825.61 ELSE 0 END)
...
SELECT  nextval('model_draft.ego_grid_hv_extension_line_id'),
	...
	topo,
	v0,
	ST_Length(ST_GeographyFromText(ST_AsEWKT(geom)))/1000,
	s_min
FROM model_draft.ego_grid_pf_hv_extension_transformer WHERE v0 = v1;*/
...
SELECT 'extension_nep2035_b2', bus_id, v_nom, geom, project, bus_name FROM model_draft.ego_grid_pf_hv_extension_bus WHERE scn_name = 'extension_NEP';
...
INSERT INTO model_draft.ego_grid_pf_hv_extension_bus (scn_name, bus_id, v_nom, geom, bus_name, project)
	SELECT DISTINCT ON (osm_name) 
		...
		osm_name,
		'B2'
	FROM grid.otg_ehvhv_bus_data a, grid.scn_nep2035_b2_line b WHERE osm_name =  b.startpunkt OR osm_name = b.endpunkt OR osm_name = 'Güstrow' AND base_kv = 380;
...
INSERT INTO model_draft.ego_grid_pf_hv_extension_line (scn_name, line_id, bus0, bus1, v_nom, s_nom, project, nova, cables) 
	SELECT
		...
		nova,
		cables
	FROM grid.scn_nep2035_b2_line WHERE scn_name = 'extension_NEP2035_B2' AND project NOT IN ('P40', 'P200');
...
INSERT INTO model_draft.ego_grid_pf_hv_extension_line (scn_name, line_id, s_nom, topo, geom,bus0, bus1, project, v_nom)
	...
	FROM grid.scn_nep2035_b2_line a, model_draft.ego_grid_pf_hv_line b WHERE a.scn_name = 'decommissioning_NEP2035_B2' 
...
SELECT	nextval('model_draft.ego_grid_hv_extension_link_id'),
	geom,
	p_nom,
	bus1,
	bus0,
...
UPDATE model_draft.ego_grid_pf_hv_extension_transformer a
SET	capital_cost = (CASE v0 WHEN 220 THEN 825.61 WHEN 110 THEN 1010.15 ELSE 0 END),
	s_nom_min = 0,
	s_nom_max = s_nom
WHERE scn_name = 'extension_nep2035_b2';
...
UPDATE model_draft.ego_grid_pf_hv_extension_line
SET capital_cost = (CASE WHEN nova = 'Netzverstärkung: Stromkreisauflage/Umbeseilung' AND v_nom = 220 THEN (5.62 * length )
			WHEN nova = 'Netzverstärkung: Stromkreisauflage/Umbeseilung' AND v_nom = 380 THEN (4.92 * length)
			WHEN nova = 'Netzverstärkung: Neubau in bestehender Trasse' AND v_nom = 380 THEN (19.67 * length )
			WHEN nova = 'Netzausbau: Neubau in neuer Trasse' AND v_nom = 380 THEN (18.44 * length )
		ELSE 0 END);
...
SET capital_cost = (CASE WHEN project_id = 1 THEN 128.21 * length
			WHEN project_id = 2 THEN 17.48 * length
			WHEN project_id = 3 THEN 128.21 * length
			WHEN project_id = 30 THEN 128.21 * length
			WHEN project_id = 33 THEN 27.68 * length 
			WHEN project_id = 4 THEN 128.21 * length
			WHEN project_id = 5 THEN 128.21 * length
			WHEN project_id = 29 THEN 27.68 * length 
...
---DELETE SELECT * FROM model_draft.ego_grid_pf_hv_extension_transformer  WHERE x IS NULL;

UPDATE model_draft.ego_grid_pf_hv_extension_bus
SET current_type = 'AC';

---SELECT * FROM model_draft.ego_grid_pf_hv_extension_line WHERE x is null
