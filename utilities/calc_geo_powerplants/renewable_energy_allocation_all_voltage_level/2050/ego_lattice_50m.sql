...
DROP TABLE IF EXISTS  	model_draft.ego_lattice_50m_2050 CASCADE;
CREATE TABLE         	model_draft.ego_lattice_50m_2050 (
		id SERIAL NOT NULL,
		subst_id integer,
		area_type text,
...
-- insert lattice
INSERT INTO     model_draft.ego_lattice_50m_2050 (geom_box)
...
CREATE INDEX 	ego_lattice_50m_geom_box_idx
	ON 	model_draft.ego_lattice_50m_2050 USING gist (geom_box);

-- centroid
UPDATE 	model_draft.ego_lattice_50m_2050
	SET	geom = ST_CENTROID(geom_box);

-- index gist (geom)
CREATE INDEX 	ego_lattice_50m_geom_idx
	ON 	model_draft.ego_lattice_50m_2050 USING gist (geom);

-- grant (oeuser)
ALTER TABLE	model_draft.ego_lattice_50m_2050 OWNER TO oeuser;

-- metadata
COMMENT ON TABLE model_draft.ego_lattice_50m_2050 IS '{
...
-- select description
SELECT obj_description('model_draft.ego_lattice_50m_2050' ::regclass) ::json;

-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.3','output','model_draft','ego_lattice_50m_2050','ego_lattice_50m.sql',' ');
