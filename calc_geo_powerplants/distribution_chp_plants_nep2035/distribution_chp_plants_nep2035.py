...
meta.reflect(bind=engine_home, schema='supply'\
            ,only=['ego_renewable_power_plants_germany'])
            
# Map to classes
Base = automap_base(metadata=meta)
Base.prepare()
          
SQL_Data = Base.classes.ego_renewable_power_plants_germany
...
if (answer == 'j'):
    ...
    distribution_energy = status_quo_overall_performance#kwk_energy_until_2035
...
# Collection data from the database
meta.reflect(bind=engine_home, schema='calc_ego_loads',\
                                                    only=['ego_deu_load_area'])
            
# Map to classes
Base = automap_base(metadata=meta)
Base.prepare()
          
SQL_Data = Base.classes.ego_deu_load_area
...
if(answer_result == 1):
    ...
    with open('chp_distribution.csv', 'w') as csvfile:
        fieldnames = ['id',\
                      'start_up_date' ,\
                      'electrical_capacity',\
                      'generation_type',
