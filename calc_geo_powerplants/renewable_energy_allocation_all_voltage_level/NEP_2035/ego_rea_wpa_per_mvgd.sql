...
-- wind potential area
DROP TABLE IF EXISTS  	model_draft.ego_supply_wpa_2035 CASCADE;
CREATE TABLE         	model_draft.ego_supply_wpa_2035 (
		id SERIAL NOT NULL,
...
-- insert wpa dump
INSERT INTO     model_draft.ego_supply_wpa_2035 (geom)
...
CREATE INDEX 	ego_supply_wpa_geom_idx
	ON 	model_draft.ego_supply_wpa_2035 USING gist (geom);

-- grant (oeuser)
ALTER TABLE	model_draft.ego_supply_wpa_2035 OWNER TO oeuser;

-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.3','temp','model_draft','ego_supply_wpa_2035','ego_rea_wpa_per_mvgd.sql',' ');

/* -- validate (geom)
DROP VIEW IF EXISTS	model_draft.ego_supply_wpa_error_geom_view_2035 CASCADE;
CREATE VIEW		model_draft.ego_supply_wpa_error_geom_view_2035 AS 
	SELECT	test.id,
		test.error,
	...
	FROM	(
		SELECT	source.id AS id,				-- PK
			...
			source.geom AS geom
		FROM	model_draft.ego_supply_wpa_2035 AS source	-- Table
		) AS test
...
-- wpa per mv-griddistrict
DROP TABLE IF EXISTS  	model_draft.ego_supply_wpa_per_mvgd_2035 CASCADE;
CREATE TABLE         	model_draft.ego_supply_wpa_per_mvgd_2035 (
		id SERIAL NOT NULL,
		subst_id integer,
		area_ha decimal,
		geom geometry(Polygon,3035),
...
INSERT INTO     model_draft.ego_supply_wpa_per_mvgd_2035 (area_ha, geom)
	...
	FROM	(SELECT ST_MakeValid((ST_DUMP(ST_MULTI(ST_SAFE_INTERSECTION(pot.geom,dis.geom)))).geom) AS geom
		FROM	model_draft.ego_supply_wpa_2035 AS pot,
...
-- substation id
UPDATE 	model_draft.ego_supply_wpa_per_mvgd_2035 AS t1
SET  	subst_id = t2.subst_id
FROM    (
	SELECT	pot.id AS id,
		gd.subst_id AS subst_id
	FROM	model_draft.ego_supply_wpa_per_mvgd_2035 AS pot,
...
CREATE INDEX 	ego_supply_wpa_per_mvgd_geom_idx
	ON 	model_draft.ego_supply_wpa_per_mvgd_2035 USING gist (geom);

-- grant (oeuser)
ALTER TABLE	model_draft.ego_supply_wpa_per_mvgd_2035 OWNER TO oeuser;

-- metadata
COMMENT ON TABLE model_draft.ego_supply_wpa_per_mvgd_2035 IS '{
...
-- select description
SELECT obj_description('model_draft.ego_supply_wpa_per_mvgd_2035' ::regclass) ::json;

-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.3','output','model_draft','ego_supply_wpa_per_mvgd_2035','ego_rea_wpa_per_mvgd.sql',' ');
