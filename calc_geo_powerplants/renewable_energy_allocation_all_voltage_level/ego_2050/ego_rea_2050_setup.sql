...
The methode is used for the 2050 open_ego scenarion data set
Used script version dev/ from 21.02.2017

__copyright__ 	= "Reiner Lemoine Institut gGmbH"
__license__ 	= "GNU Affero General Public License Version 3 (AGPL-3.0)"
__url__ 	= "https://github.com/openego/data_processing/blob/master/LICENSE"
__author__ 	= "Ludee, wolfbunke"
...
DROP TABLE IF EXISTS 	model_draft.ego_supply_rea_2050 CASCADE;
...
CONSTRAINT ego_supply_rea_2050_pkey PRIMARY KEY (id));
...
-- index GIST (geom)
CREATE INDEX	ego_supply_rea_2050_geom_idx
	ON	model_draft.ego_supply_rea_2050 USING GIST (geom);

-- index GIST (geom_line)
CREATE INDEX ego_supply_rea_2050_geom_line_idx
	ON model_draft.ego_supply_rea_2050 USING gist (geom_line);

-- index GIST (geom_new)
CREATE INDEX ego_supply_rea_2050_geom_new_idx
	ON model_draft.ego_supply_rea_2050 USING gist (geom_new);

-- grant (oeuser)
ALTER TABLE model_draft.ego_supply_rea_2050 OWNER TO oeuser;

-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.3','input','model_draft','ego_grid_mv_griddistrict','ego_rea_2050_setup.sql',' ');
...
UPDATE 	model_draft.ego_supply_rea_2050 AS t1
	...
	FROM    (
		...
		FROM	model_draft.ego_supply_rea_2050 AS dea,
...
-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.3','output','model_draft','ego_supply_rea','ego_rea_2050_setup.sql',' ');
...
CREATE MATERIALIZED VIEW 		model_draft.ego_supply_rea_2050_out_mview AS
	SELECT	dea.*
	FROM 	model_draft.ego_supply_rea_2050 AS dea
	WHERE	flag = 'out' OR flag = 'offshore';

-- index GIST (geom)
CREATE INDEX ego_supply_rea_2050_out_mview_geom_idx
	ON model_draft.ego_supply_rea_2050_out_mview USING gist (geom);

-- index GIST (geom_line)
CREATE INDEX ego_supply_rea_2050_out_mview_geom_line_idx
	ON model_draft.ego_supply_rea_2050_out_mview USING gist (geom_line);

-- index GIST (geom_new)
CREATE INDEX ego_supply_rea_2050_out_mview_geom_new_idx
	ON model_draft.ego_supply_rea_2050_out_mview USING gist (geom_new);	

-- grant (oeuser)
ALTER TABLE model_draft.ego_supply_rea_2050_out_mview OWNER TO oeuser;

-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.3','input','model_draft','ego_grid_hvmv_substation','ego_rea_2050_setup.sql',' ');

-- new geom, DEA to next substation
DROP TABLE IF EXISTS	model_draft.ego_supply_rea_2050_out_nn CASCADE;
CREATE TABLE 		model_draft.ego_supply_rea_2050_out_nn AS 
	...
	FROM 	model_draft.ego_supply_rea_2050_out_mview AS dea,
...
ALTER TABLE	model_draft.ego_supply_rea_2050_out_nn
...
UPDATE 	model_draft.ego_supply_rea_2050 AS t1
	...
	FROM	(SELECT	nn.dea_id AS dea_id,
		...
		FROM	model_draft.ego_supply_rea_2050_out_nn AS nn,
			model_draft.ego_supply_rea_2050 AS dea
		WHERE  	flag = 'out'
		)AS t2
...
-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.3','temp','model_draft','ego_supply_rea_2050_out_mview','ego_rea_2050_setup.sql',' ');
...
-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.3','input','model_draft','ego_osm_sector_per_griddistrict_4_agricultural','ego_rea_2050_setup.sql',' ');
...
-- ego scenario log (version,io,schema_name,table_name,script_name,comment)
SELECT ego_scenario_log('v0.2.3','output','model_draft','ego_osm_agriculture_per_mvgd','ego_rea_2050_setup.sql',' ');
...
-- MView BNetzA
DROP MATERIALIZED VIEW IF EXISTS 	model_draft.ego_supply_rea_2050_bnetza_mview CASCADE;
CREATE MATERIALIZED VIEW 		model_draft.ego_supply_rea_2050_bnetza_mview AS
	SELECT	dea.*
	FROM 	model_draft.ego_supply_rea_2050 AS dea
	WHERE	flag = 'bnetza';

CREATE INDEX ego_supply_rea_2050_bnetza_mview_geom_idx
	ON model_draft.ego_supply_rea_2050_bnetza_mview USING gist (geom);
